...
Remember: russell command uploads contents of your current directory. So use relative paths only. '/output' is a special path. Contents stored here will be saved after
...
### To evaluate on russellcloud

    russell run --data <output_id_of_training> "python evaluate.py --checkpoint /input/model.ckpt \
      --in-path dir/of/test/imgs/ \
      --out-path dir/for/results/"
...
Russell requirements are specified in the `russell_requirements.txt` file. They will be installled before running your code.
